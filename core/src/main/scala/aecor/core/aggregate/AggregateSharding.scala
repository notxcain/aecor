...
class AggregateSharding(system: ExtendedActorSystem) extends Extension {
  ...
  abstract class MkStart[Command[_]] {
    def apply[Aggregate, State, Event](aggregate: Aggregate, settings: AggregateShardingSettings = AggregateShardingSettings(system))
                                      (implicit
  ...
  def start[Command[_]]: MkStart[Command] = new MkStart[Command] {
    ...
    (aggregate: Aggregate, settings: AggregateShardingSettings)
    (implicit aab: Aux[Aggregate, Command, State, Event], Command: ClassTag[Command[_]], Event: ClassTag[Event], State: ClassTag[State], correlation: Correlation[Command[_]], aggregateName: AggregateName[Aggregate]): AggregateRegionRef[Command] = {
