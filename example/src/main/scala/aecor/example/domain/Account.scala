...
import aecor.example.domain.Account.{AccountCredited, AccountOpened, AuthorizeTransaction, CaptureTransaction, CreditAccount, Event, Open, OpenAccount, State, TransactionAuthorized, TransactionCaptured, TransactionVoided, VoidTransaction}
...
object Account {
  ...
  case class AuthorizeTransaction(accountId: AccountId, transactionId: TransactionId, amount: Amount) extends Command[AggregateResponse[Rejection]]
  ...
  implicit def behavior: AggregateBehavior.Aux[Account, Command, State, Event] =
    new AggregateBehavior[Account, Command] {
      override type Event = Account.Event
      override type State = Account.State

      override def init(a: Account): State = Initial
