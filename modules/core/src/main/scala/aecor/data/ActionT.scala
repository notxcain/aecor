...
final class ActionT[F[_], S, E, A] private (
  ...
  def map[B](f: A => B)(implicit F: Functor[F]): ActionT[F, S, E, B] = ActionT {
    (s, u, es) =>
...
object ActionT  extends ActionNFunctions with ActionNInstances {
...
trait ActionNFunctions {
  ...
  def sample[F[_]: Monad, S, E1, Env, E2](getEnv: F[Env])(
    update: (Env, E1) => E2
  )(extract: E2 => E1): ActionT[F, S, E1, ?] ~> ActionT[F, S, E2, ?] =
  ...
  def xmapState[F[_]: Functor, S1, S2, E](update: (S2, S1) => S2)(extract: S2 => S1): ActionT[F, S1, E, ?] ~> ActionT[F, S2, E, ?] =
    new (ActionT[F, S1, E, ?] ~> ActionT[F, S2, E, ?]) {
      override def apply[A](fa: ActionT[F, S1, E, A]): ActionT[F, S2, E, A] = fa.xmapState(update)(extract)
    }
}

trait ActionNInstances extends ActionNLowerPriorityInstances {
  implicit def monadActionInstance[F[_], S, E, R](implicit F: MonadError[F, R]): MonadAction[ActionT[F, S, E, ?], F, S, E, R] =
    new ActionNMonadActionLiftInstance[F, S, E]() with MonadAction[ActionT[F, S, E, ?], F, S, E, R]  {
      override def reject[A](r: R): ActionT[F, S, E, A] = liftF(F.raiseError(r))
      override def handleErrorWith[A](fa: ActionT[F, S, E, A])(
        f: R => ActionT[F, S, E, A]
      ): ActionT[F, S, E, A] = ActionT {
        (s, u, es) =>
...
trait ActionNLowerPriorityInstances {
  abstract class ActionNMonadActionLiftInstance[F[_], S, E](
    ...
    override def tailRecM[A, B](a: A)(
      f: A => ActionT[F, S, E, Either[A, B]]
    ): ActionT[F, S, E, B] = ActionT { (s, ue, es) =>
      F.tailRecM(a) { a =>
