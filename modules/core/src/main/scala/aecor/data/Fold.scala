...
final case class Fold[F[_], A, X](initial: A, reduce: (A, X) => F[A]) {
  ...
  def expand[B](init: A => B, read: B => A, update: (B, A) => B)(implicit
      F: Functor[F]
  ): Fold[F, B, X] =
  ...
  def orWith[B, Y, AB, XY](
      that: Fold[F, B, Y]
  )(anaAB: (A, B) => AB, cataAB: AB => (A, B), cataXY: XY => Either[X, Y])(implicit
      F: Functor[F]
  ): Fold[F, AB, XY] =
    Fold(
      anaAB(initial, that.initial),
      { (ab, xy) =>
        val (a, b) = cataAB(ab)
        cataXY(xy) match {
...
object Fold {
  def optional[F[_]: Functor, A, B](empty: B => F[A])(f: (A, B) => F[A]): Fold[F, Option[A], B] =
    Fold(
      initial = Option.empty,
      reduce = { (oa: Option[A], b: B) =>
