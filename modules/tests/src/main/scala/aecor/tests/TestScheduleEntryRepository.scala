...
object TestScheduleEntryRepository {
  def apply[F[_]: MonadState[?[_], S], S](lens: Lens[S, Vector[ScheduleEntry]]): ScheduleEntryRepository[F] =
    new TestScheduleEntryRepository(lens)
}

class TestScheduleEntryRepository[F[_]: MonadState[?[_], S], S](lens: Lens[S, Vector[ScheduleEntry]])
